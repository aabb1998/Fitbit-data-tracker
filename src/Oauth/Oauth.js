...
server.register(require('bell'), function(err) {
    server.auth.strategy('fitbit', 'bell', {
        ...
        cookie: 'bell-fitbit',
        isSecure: false // Remove if server is HTTPS
    });
});

// Page to start the Authorization
server.route({
    method: 'GET',
    path:'/',
...
server.route({
    ...
    config: {
        ...
        handler: function (request, reply) {
            ...
            // set key for this database record to the user's id
...
// Refresh Fitbit Web API OAuth 2.0 access tokens
